...
minimax :: (Color -> Board -> Int) -> (Board -> Color -> [Move]) -> ([Move] -> [Move]) -> Board -> Color -> Color -> Int -> Int -> Int -> Bool -> Maybe (Int, Move)
minimax boardScorer nextStateGen movesTransformer board scoringColor playerColor n alpha beta maximising =
  let extractMove previousBestScore mPreviousBestMove = do
      ...
      minimaxInner _ _ _ Nothing [m] = Just (boardScorer scoringColor $ movePiece board m, m)
      minimaxInner a b previousBestScore mPreviousBestMove (m : ms) =
        if n == 1
          then
            let s = boardScorer scoringColor $ movePiece board m
